...
//  private staticURL = 'http://' + window.location.hostname+':8080';
  ...
  getAllUsers(start, end): Observable<any> {
    return this.http.get(`${this.staticURL}` + '/users/u/' + start + '/' + end, {responseType: 'json'});
  }
  ...
  logout(username: string): Observable<any> {
    console.log("logging out admin now");
    return this.http.post(`${this.staticURL}` + '/admin/logout', username , {responseType: 'json'});
  }
   ...
   */
  getUserInformation(spNumber : string ): Observable<any> {
    return this.http.get(`${this.staticURL}` + '/applicants/byspNumber/'  + spNumber, {responseType: 'json'});
  }
  ...
  gradeUserCV(spNumber : string ): Observable<any> {
    return this.http.post(`${this.staticURL}` + `/grader/grade/${spNumber}`, {responseType: 'json'});
  }
