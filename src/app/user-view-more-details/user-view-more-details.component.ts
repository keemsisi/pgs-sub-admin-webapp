...
import { DataObjectModel } from '../models/dataobject.model';
@Component({
...
export class UserViewMoreDetailsComponent implements OnInit {
  ...
  dataSource = new DataObjectModel().model;
  ...
  ngOnInit() {
    this.activatedRoute.params.subscribe(params => {
      ...
      this.getUserInformation(this.username); // get the user information 
    });
  }
  ...
  getUserInformation(username): void {
    ...
    this.httpRequest.getUserInformation(username).subscribe(data => {
      this.dataSource = JSON.parse(data.cv); // assigns the username ;

      this.personalInformation = [this.dataSource['personalInformation'] , this.dataSource['personalInformation'] ];
      // this.contactAddresses = [this.dataSource['personalInformation']['contactAddresses'][0]];
      this.contactAddresses =  [{contactAddress: "11, Ajagungbade Street , Mafoluku Oshodi Lagos"}];
      this.phoneNumbers = [this.dataSource['personalInformation']['phoneNumbers']];
      this.emailAddresses = [this.dataSource['personalInformation']['emailAddresses']];

 
      console.log(this.dataSource['eaphni']['educationArray']);
      // console.log(this.contactAddresses);
      // console.log(this.phoneNumbers);
      // console.log(this.emailAddresses);


      // console.log(this.userInfo['masterFormGroupings']['extraCurriculaActivities']);
